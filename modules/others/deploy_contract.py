from modules import *
from utils.client import Client
from modules.interfaces import *


class DeployContractInkWorker(Logger):
    def __init__(self, client: Client, module_info: DeployContractInkModule):
        super().__init__()
        self.client: Client = client

    async def run(self):
        self.logger.info(
            f'{self.client.name} Deployment of the contract in the Ink network'
        )

        data = ''
        
        try:
            tx_params = await self.client.prepare_transaction(value=200000000000000, eip1559=False)
            transaction = {**tx_params, 'data': data}
            await self.client.send_transaction(transaction, need_hash=True)
        except Exception as error:
            self.logger.error(
                f'{self.client.name} Deployment Error: {error} '
            )